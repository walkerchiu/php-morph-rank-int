...
namespace WalkerChiu\MorphRank\Models\Observers;

class StatusLangObserver
{
    /**
    ...
    {
        $query =
            config('wk-core.class.morph-rank.statusLang')
                ::withTrashed()
        ...
        if (
            config('wk-morph-rank.soft_delete')
            && (
                config('wk-core.lang_log')
                || config('wk-morph-rank.lang_log')
            )
        ) {
